...
class Api::V1::PagesControllerTest < Api::Test
  def setup
    ...
    @page = build(:page, site: @site)
    @other_pages = create_list(:page, 3)

    @another_page = create(:page, site: @site)
  ...
  def assert_proper_object_serialization(page_data)
    ...
    assert_equal page_data["site_id"], page.site_id
  end

  test "index" do
    # Fetch and ensure nothing is seriously broken.
    get "/api/v1/sites/#{@site.id}/pages", params: {access_token: access_token}
  ...
  test "create" do
    ...
    page_data = JSON.parse(build(:page, site: nil).api_attributes.to_json)
    page_data.except!("id", "site_id", "created_at", "updated_at")
    params[:page] = page_data

    post "/api/v1/sites/#{@site.id}/pages", params: params
    assert_response :success
